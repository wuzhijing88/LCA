...
def create_process_workflow_runtime(**kwargs):
    """Compatibility seam retained for existing UI/runtime tests."""
    return create_coordinated_workflow_runtime(
    ...
    def _create_executor(self):
        ...
        if target_hwnd or target_window_title or self.bound_window_id:
            has_explicit_match = any(
                ...
                and (
                    (target_hwnd and as_hwnd(window.get("hwnd")) == as_hwnd(target_hwnd))
                    or (
                        self.bound_window_id
                        and str(window.get("bind_id") or "") == str(self.bound_window_id)
                    )
                    or (
                        target_window_title
                        and str(window.get("title") or "") == str(target_window_title)
                    )
                )
                for window in effective_bound_windows
            )
            if target_hwnd and not has_explicit_match:
                raise ValueError(
