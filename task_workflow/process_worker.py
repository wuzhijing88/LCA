...
class _SocketSignalBridge:
    ...
    def __init__(self, sock: socket.socket, auth_token: str = ""):
        self._sock = sock
    ...
    def send_ready(self) -> bool:
        return self._send({"type": "ready", "auth_token": self._auth_token})

    def send_execution_finished(self, success: bool, message: str) -> bool:
    ...
    def bind_executor(self, executor_obj: Any) -> None:
        ...
        if finished_signal is not None:
            ...
            def _on_finished(success: bool, message: str):
                if not self.send_execution_finished(success, message):
...
def _materialize_workflow_reference(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_cards = workflow_data.get("cards") or {}
...
def run_workflow_worker_standalone(port: int) -> int:
    ...
    try:
        ...
        auth_token = str(os.getenv("LCA_WORKFLOW_AUTH_TOKEN", "") or "")
        if not auth_token:
