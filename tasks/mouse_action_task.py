        ...
        if (effective_mode or '').startswith('foreground'):
            try:
                ...
                if driver_type == 'ibinputsimulator' and active_driver is not None:
                    # 键鼠分离时鼠标移动只关心鼠标侧驱动
                    mouse_side = getattr(active_driver, 'mouse_driver', None) or active_driver
                    selected_driver = getattr(mouse_side, '_driver_name', None)
                    if selected_driver:
...
def _correct_image_paths(raw_image_paths: list) -> list:
    ...
    corrected = correct_image_paths(parsed_paths, card_id=None)

    # 多图路径是一个集合语义：同一文件重复出现时，识别记忆（set）无法
    # 区分重复项，最终会导致“全部点击”永远达不到图片总数。保留首次出现
    # 的顺序，同时按 Windows 路径规则去重；memory:// 资源按 URI 精确去重。
    unique_paths = []
    seen = set()
    for path in corrected:
        ...
        if text.startswith("memory://"):
            ...
            identity = os.path.normcase(os.path.normpath(os.path.abspath(text)))
        if identity in seen:
            continue
        seen.add(identity)
        unique_paths.append(text)
    if len(unique_paths) != len(corrected):
        logger.info("[多图路径] 已移除重复图片: %d -> %d", len(corrected), len(unique_paths))
    return unique_paths
