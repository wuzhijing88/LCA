...
def get_params_definition() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "countdown_data": {
            ...
            "type": "text",
            "default": "00:00:30",
            "tooltip": "填写 JSON（如 {\"HH\":0,\"MM\":14,\"SS\":30}）、HH:MM:SS，或秒数。",
            "condition": [
                {"param": "condition_type", "value": "时间判断"},
        ...
        "countdown_field_hour": {
            ...
            "type": "text",
            "default": "HH",
            "tooltip": "JSON 中表示小时的字段名，留空表示无小时字段",
            "condition": [
                {"param": "condition_type", "value": "时间判断"},
                {"param": "time_mode", "value": "倒计时判断"}
            ]
        ...
        "countdown_field_minute": {
            ...
            "type": "text",
            "default": "MM",
            "tooltip": "JSON 中表示分钟的字段名，留空表示无分钟字段",
            "condition": [
                {"param": "condition_type", "value": "时间判断"},
                {"param": "time_mode", "value": "倒计时判断"}
            ]
        ...
        "countdown_field_second": {
            ...
            "type": "text",
            "default": "SS",
            "tooltip": "JSON 中表示秒的字段名，留空表示无秒字段",
            "condition": [
                {"param": "condition_type", "value": "时间判断"},
                {"param": "time_mode", "value": "倒计时判断"}
            ]
...
def _parse_countdown_literal(raw: Any) -> Tuple[bool, Any, str]:
    """把倒计时输入当字面量解析。成功返回 (True, data, "")，失败返回 (False, None, 原因)。"""
    if raw is None:
        return False, None, "未找到倒计时数据或格式不支持"
    if isinstance(raw, bool):
        return False, None, f"倒计时格式不支持: {type(raw)}"
    ...
    if (text.startswith("{") and text.endswith("}")) or (text.startswith("[") and text.endswith("]")):
        ...
        if isinstance(parsed, bool):
            return False, None, f"倒计时格式不支持: {type(parsed)}"
        if isinstance(parsed, (dict, list, int, float)):
            return True, parsed, ""
        return False, None, f"倒计时格式不支持: {type(parsed)}"
    ...
    if ":" in text:
        ...
        return True, {"HH": hours, "MM": minutes, "SS": seconds}, ""
...
def _evaluate_time_condition(params: Dict[str, Any], counters: Dict[str, int], card_id: Optional[int]) -> Tuple[bool, str]:
    ...
    try:
        ...
        if time_mode == '到达指定时间':
            ...
            if missing_keys:
                return False, f"倒计时配置缺少字段: {', '.join(missing_keys)}"
            if "countdown_data" not in params:
                return False, "倒计时配置缺少字段: countdown_data"
            countdown_raw = params["countdown_data"]
            if isinstance(countdown_raw, str) and not countdown_raw.strip():
                return False, "倒计时输入为空"
            if countdown_raw is None:
                return False, "倒计时输入为空"
            field_hour = str(params["countdown_field_hour"] or "").strip()
            field_minute = str(params["countdown_field_minute"] or "").strip()
            field_second = str(params["countdown_field_second"] or "").strip()
            ...
            if timer_key in counters:
                # 计时器已启动，计算剩余时间
                if start_time_key not in counters or total_seconds_key not in counters:
                    return False, "倒计时计时器数据缺少必要字段"
                start_time = counters[start_time_key]
                total_seconds = counters[total_seconds_key]

                if start_time is None:
                    return False, "倒计时计时器起始时间为空"
                else:
                    # 计算已过去的时间
            ...
            # 首次执行，把输入当字面量解析并启动计时器
            try:
                countdown_found, countdown_data, parse_error = _parse_countdown_literal(
                    countdown_raw
                )

                if not countdown_found:
                    msg = parse_error or "未找到倒计时数据或格式不支持"
                    logger.warning(f"[时间判断] {msg}")
                    return False, msg
                ...
                if isinstance(countdown_data, dict):
                    # 字典格式，按字段名提取
                    if field_hour:
                        if field_hour not in countdown_data:
                            return False, f"倒计时数据缺少字段: {field_hour}"
                        hours = int(countdown_data[field_hour])
                    if field_minute:
                        if field_minute not in countdown_data:
                            return False, f"倒计时数据缺少字段: {field_minute}"
                        minutes = int(countdown_data[field_minute])
                    if field_second:
                        if field_second not in countdown_data:
                            return False, f"倒计时数据缺少字段: {field_second}"
                        seconds = int(countdown_data[field_second])
                elif isinstance(countdown_data, list):
                    # 列表格式，按索引提取
                    if len(countdown_data) != 3:
                        return False, "倒计时列表必须包含时、分、秒三个元素"
                    ...
                    msg = f"倒计时格式不支持: {type(countdown_data)}"
