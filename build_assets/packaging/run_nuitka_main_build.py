...
def _validate_ocr_build_inputs(project_root: Path) -> None:
    ...
    requirements_path = project_root / "requirements-runtime.txt"
...
def _allowed_ocr_model_paths(dist_dir: Path) -> set[Path]:
    model_dir = dist_dir / OCR_MODEL_DIRECTORY
    return {
        (model_dir / filename).resolve(strict=False)
        for filename, _expected_hash in OCR_MODEL_FILES.values()
    }
...
def _remove_unused_qt_editor_runtime(dist_dir: Path) -> list[tuple[Path, int]]:
    ...
    for path in sorted(dist_dir.rglob("*"), key=lambda item: len(item.parts), reverse=True):
        relative_path = path.relative_to(dist_dir)
        if not _is_unused_qt_editor_path(relative_path):
            continue
        if path.is_file():
            ...
            continue
        if path.is_dir():
            leftover_dirs.append(path)

    for directory in leftover_dirs:
        if directory.exists():
            _remove_tree(directory)
            removed.append((directory.relative_to(dist_dir), 0))
    return removed


def _remove_unexpected_ocr_models(dist_dir: Path) -> list[tuple[Path, int]]:
    if not dist_dir.is_dir():
        return []

    allowed_paths = _allowed_ocr_model_paths(dist_dir)
    extra_suffixes = {".onnx", ".mnn", ".pdmodel", ".pdiparams", ".pth"}
    removed: list[tuple[Path, int]] = []
    for file_path in sorted(path for path in dist_dir.rglob("*") if path.is_file()):
        if file_path.suffix.lower() not in extra_suffixes:
            continue
        if file_path.resolve(strict=False) in allowed_paths:
            continue
        file_size = int(file_path.stat().st_size)
        relative_path = file_path.relative_to(dist_dir)
        file_path.unlink()
...
def main() -> int:
    ...
    if removed_unused_qt_runtime:
        removed_size = sum(file_size for _relative_path, file_size in removed_unused_qt_runtime)
        print(
            "removed_unused_qt_editor_runtime="
            f"{len(removed_unused_qt_runtime)} items, {removed_size / 1024 / 1024:.2f} MB"
        )
        for relative_path, _file_size in removed_unused_qt_runtime:
            print(f"  - {relative_path.as_posix()}")

    removed_unexpected_models = _remove_unexpected_ocr_models(result_exe.parent)
    if removed_unexpected_models:
        removed_size = sum(file_size for _relative_path, file_size in removed_unexpected_models)
        print(
            "removed_unexpected_ocr_models="
            f"{len(removed_unexpected_models)} files, {removed_size / 1024 / 1024:.2f} MB"
        )
        for relative_path, _file_size in removed_unexpected_models:
            print(f"  - {relative_path.as_posix()}")
