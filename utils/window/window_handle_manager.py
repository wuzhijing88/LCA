...
class WindowHandleManager:
    ...
    def _capture_window_snapshot(self, hwnd: int, title: str = "") -> Dict[str, Any]:
        ...
        try:
            from utils.window.window_identity import capture_window_identity
            import win32gui
            ...
            identity = capture_window_identity(hwnd_int)
            if identity.get('title'):
                snapshot['title'] = identity['title']
            snapshot['class_name'] = identity.get('class_name', '')
            snapshot['process_name'] = identity.get('process_name', '')
    ...
    def _find_window_by_info(self, window_info: WindowInfo) -> Optional[int]:
        """根据窗口稳定特征重新查找窗口句柄"""
        try:
            from utils.window.window_identity import resolve_bound_window_hwnd

            found_hwnd = resolve_bound_window_hwnd({
    ...
    def _update_window_hwnd(self, key: str, new_hwnd: int):
        ...
        with self._lock:
            if key in self._window_registry:
                new_hwnd = as_hwnd(new_hwnd)
                old_hwnd = as_hwnd(self._window_registry[key].hwnd)
                ...
                self._window_registry[key].process_name = snapshot.get('process_name', '')
