...
def is_packaged_runtime() -> bool:
    """Detect Nuitka/frozen packaging from explicit runtime markers."""
    if bool(getattr(sys, "frozen", False)):
        return True
    main_module = sys.modules.get("__main__")
    return main_module is not None and hasattr(main_module, "__compiled__")


def resolve_main_executable() -> Optional[str]:
    """Return the current packaged executable without probing alternatives."""
    executable = os.path.abspath(str(sys.executable or "").strip())
...
def build_worker_launch_command(
    ...
    *,
    ...
    if is_packaged_runtime():
        ...
        if not executable:
            raise FileNotFoundError("packaged main executable not found")
        return [executable, flag, *resolved_extra_args]

    resolved_python = str(python_executable or "").strip()
    if not resolved_python:
        resolved_python = resolve_project_python_executable(project_root=project_root)
