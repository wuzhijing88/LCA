...
class ParameterPanelWidgetRegistryMixin:
    ...
    def _register_workflow_dependent_refresher(self, parameter_name: Optional[str], callback) -> None:
        if not parameter_name or not callable(callback):
            return
        self._workflow_dependent_refreshers.setdefault(parameter_name, []).append(callback)

    def _refresh_workflow_dependents(self, parameter_name: str) -> None:
        for refresher in list(self._workflow_dependent_refreshers.get(parameter_name, [])):
            try:
                refresher()
            except Exception as exc:
                logger.debug("refresh dependent selector failed: %s", exc)
