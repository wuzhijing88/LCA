    ...
    def _create_widgets(self):
        ...
        for name, param_def in self.param_definitions.items():
            ...
            if widget_hint == 'colorpicker':
                ...
                saved_workflow_value = current_value
                try:
                    normalized_workflow_value = _normalize_workflow_value(saved_workflow_value)
                    workflow_selector_is_valid = True
                except (TypeError, ValueError):
                    normalized_workflow_value = saved_workflow_value
                    workflow_selector_is_valid = False
                index = (
                    combo_box.findData(normalized_workflow_value)
                    if workflow_selector_is_valid
                    else -1
                )
                if workflow_selector_is_valid and normalized_workflow_value is None:
                    combo_box.setCurrentIndex(0)
                elif index >= 0:
                    combo_box.setCurrentIndex(index)
                else:
                    combo_box.addItem(
                        f"无效工作流（已保存）: {saved_workflow_value!s}",
                        saved_workflow_value,
                    )
                ...
                desired = "" if current_value is None else str(current_value).strip()
                if desired:
