...
class MainWindowFavoritesBatchMixin:
    ...
    def _finish_batch_execute(self, success: bool = True, message: str = "Batch execute completed"):
        ...
        if task_manager is not None:
            try:
                has_active_runtime = bool(task_manager.has_active_runtime_tasks())
            except Exception as exc:
                logger.warning("批量执行结束时读取任务运行态失败: %s", exc)
        if not has_active_runtime:
            state_manager = getattr(self, 'task_state_manager', None)
            if state_manager is not None:
                try:
                    state_manager.confirm_stopped()
                    logger.info("批量队列结束且无活动任务，已确认全局停止状态")
                except Exception as exc:
                    logger.warning("批量执行结束时恢复全局停止状态失败: %s", exc)
        if hasattr(self, '_floating_controller') and self._floating_controller:
