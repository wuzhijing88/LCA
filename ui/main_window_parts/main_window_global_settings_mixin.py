...
def _sync_foreground_input_runtime(config: dict) -> None:
    ...
    legacy_backend = str(
        config.get('foreground_driver_backend', 'interception') or 'interception'
    ).strip().lower()
    mouse_backend = str(
        config.get('foreground_mouse_driver_backend', legacy_backend) or legacy_backend
    ).strip().lower()
    keyboard_backend = str(
        config.get('foreground_keyboard_driver_backend', legacy_backend) or legacy_backend
    ).strip().lower()
