...
class MainWindowUiSetupMixin:
    def _setup_main_window_ui(self):
        ...
        if not force_custom:
            if self._apply_native_rounded_corners():
                self._is_win11_rounded = True
                logger.info("已应用 Windows 11 原生圆角效果")
            else:
                logger.info("Windows 11 圆角 API 不可用，将使用自定义绘制圆角")
        else:
    ...
    def changeEvent(self, event: QEvent) -> None:
        ...
        if event.type() == QEvent.Type.WindowStateChange:
            ...
            if self.isActiveWindow():
                self._refresh_native_rounded_corners()
                try:
    ...
    def _apply_force_down_popup_to_widget(self, widget):
        ...
        return
    def _apply_native_rounded_corners(self) -> bool:
    ...
    def _refresh_native_rounded_corners(self) -> None:
        if not getattr(self, "_is_win11_rounded", False):
            return
        try:
