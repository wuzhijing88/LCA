...
class MainWindowTimerDialogMixin:
    ...
    def _build_main_clock_tab(self, dialog, tab_widget, title, enable_text, time_label, policy, spinbox_cls, combo_cls, extra_rows=None):
    ...
    def open_timer_dialog(self):
        ...
        dialog, main_layout, tab_widget = create_timer_dialog_shell(self, "\u5b9a\u65f6\u4efb\u52a1", width=680, height=480)
        ...
        probability_row = QHBoxLayout()
        probability_row.addWidget(QLabel("触发概率:"))
        probability = NoWheelSpinBox(dialog)
        probability.setRange(0, 100)
        probability.setValue(bundle.random_pause.probability)
        probability.setSuffix(" %")
        fit_timer_spinbox(probability, min_width=80)
        probability_row.addWidget(probability)
        probability_row.addStretch(1)
        random_layout.addLayout(probability_row)
