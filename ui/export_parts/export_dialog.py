...
from app_core.player.window_resolution import required_client_size_from_main
...
class StandaloneExportDialog(QDialog):
    ...
    def __init__(self, main_window, parent=None):
        ...
        self._name_edit.setPlaceholderText("安装后程序名 / 安装包名")
        self._name_edit.textChanged.connect(self._on_name_changed)
        self._name_edit.textChanged.connect(self._persist_draft)
        form.addRow("程序名称", self._name_edit)
        ...
        form.addRow("安装包保存到", output_row)

        self._icon_edit = QLineEdit(draft_icon if draft_icon and Path(draft_icon).is_file() else "")
        self._icon_edit.setPlaceholderText("可选，不选则用默认图标（程序与安装包共用）")
        ...
        self._reset_design_btn = QPushButton("重置界面")
        self._reset_design_btn.setToolTip("清除已保存的自定义界面，恢复默认（需确认）")
        self._reset_design_btn.clicked.connect(self._reset_design)
        design_row.addWidget(self._design_btn)
        design_row.addWidget(self._reset_design_btn)
        design_row.addWidget(self._design_status, 1)
        root.addLayout(design_row)
    ...
    def _refresh_design_status(self):
        if not self._has_custom_design():
            self._design_status.setText("未定制（将使用默认界面）")
            self._design_status.setStyleSheet("color: palette(mid);")
            self._reset_design_btn.setEnabled(False)
            return
        widget_count = len((self._ui_payload or {}).get("widgets") or [])
        self._design_status.setText(f"已定制界面（{widget_count} 个控件，已自动保存）")
    ...
    def _required_client_size(self) -> tuple[int, int]:
        return required_client_size_from_main(self._main)
    ...
    def _ensure_export_catalog(self, *, force_prompt: bool = False) -> bool:
        ...
        if not full:
            QMessageBox.warning(self, "无法继续", "当前工作区没有可打包的工作流（需已落盘且含卡片）。")
            return False
    ...
    def _open_designer(self):
        ...
        app_name = self._name_edit.text().strip() or "独立程序"
        catalog = self._session_catalog()
        seed_ui = apply_catalog_to_ui_exclusive(self._ui_payload or {}, catalog)
        ...
        worker = _AssembleWorker(
            {
                ...
                "required_client_width": self._required_client_size()[0],
                "required_client_height": self._required_client_size()[1],
