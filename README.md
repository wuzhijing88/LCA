...
LCA 的定位是本地、视觉驱动、低延迟、多窗口的桌面自动化引擎，不是企业 RPA 平台。项目聚焦工作流执行、窗口输入、OCR、图像识别和本地调度，不建设机器人账号中心、流程审批或云端 Bot Farm。
...
当前开发环境使用 Windows 和 Python 3.10。运行依赖见 `requirements-runtime.txt`，构建依赖见 `requirements-build.txt`，测试工具见 `requirements-dev.txt`；`requirements.txt` 保留为兼容的完整构建入口。
...
配置、工作流、图片、日志和运行态默认保存在 `%LOCALAPPDATA%\LCA`。设置 `LCA_PORTABLE=1` 可继续使用程序同目录的便携模式；`LCA_USER_DATA_DIR` 可显式指定用户数据目录。
...
运行时验证场景见 `docs/testing.md`，任务节点接入规范见 `docs/task_authoring.md`。
...
脚本使用 Nuitka 生成程序目录；本机安装 Inno Setup 6 时会继续生成安装包。模型、Interception、AutoHotkey、IbInputSimulator 和其他运行文件由受控构建环境提供，并统一通过 `build_assets/third_party/manifest.json` 哈希校验。CI 使用 `LCA_NONINTERACTIVE=1` 关闭交互提示。
