...
class MultiProcessOCRPool:
    ...
    def _terminate_process_tree(self, process: Optional[subprocess.Popen], wait_timeout: float = 1.0) -> None:
        ...
        timeout_value = float(wait_timeout)
        try:
            if os.name == "nt":
                ...
                if result.returncode != 0 and process.poll() is None:
                    raise OCRProcessTerminationError(
                        pid, f"taskkill exited with code {result.returncode}"
                    )
            else:
                process.terminate()

            process.wait(timeout=timeout_value)
        except OCRProcessTerminationError:
            raise
        except subprocess.TimeoutExpired as exc:
            raise OCRProcessTerminationError(pid, "termination timed out") from exc
        except (OSError, subprocess.SubprocessError) as exc:
            raise OCRProcessTerminationError(pid, str(exc)) from exc

        if process.poll() is None:
            raise OCRProcessTerminationError(pid, "process is still running")
        unregister_worker_process(pid)
